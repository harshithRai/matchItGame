...
import 'matchViewModel.dart';

class MatchMobilePortraitView extends StatelessWidget {
  const MatchMobilePortraitView({Key key}) : super(key: key);
  @override
                    ...
                    children: [
                              ...
                              children: [
                                Column(
                                  ...
                                  children: model.matchCards.map<Widget>(
                                        (e) {
                                          keyIndex =
                                              model.matchCards.indexOf(e);
                                          return AbsorbPointer(
                                            absorbing: e.isSourcePlayed,
                                            child: Container(
                                  ...
                                  children: model.matchCards.map<Widget>(
                                        (e) {
                                          keyIndex =
                                              model.matchCards.indexOf(e);
                                          return AbsorbPointer(
                                            absorbing: e.isDestinationPlayed,
                                            child: Container(
                                              decoration: BoxDecoration(
                                                border: Border.all(
                                                  width: 0.0,
                      ...
                      CustomPaint(
                        foregroundPainter: ShapesPainter(
                          sourceOffsetList: model.sourceOffsetList,
                          matchOffsetList: model.matchOffsetList,
                          sourceCardSize: model.sourceCardSize,
                          matchCardSize: model.matchCardSize,
                          // lineColor: Theme.of(context).accentColor,
                          lineColors: model.lineColors,
                        ),
                      ),
                    ],
                  ),
  ...
  DragTarget<String> buildDragTargetCards(
      MatchViewModel model, MatchCard e, String side, String cardType) {
    return DragTarget(
        ...
        return Container(
          ...
          child: Stack(
            children: [
              ...
              if (((cardType == 'source' && e.isSourcePlayed) ||
                      (cardType == 'match' && e.isDestinationPlayed)) &&
                  e.isMatched)
                Positioned.fill(
                  child: Align(
      ...
      onWillAccept: (data) {
        ...
        bool flag = false;
        if (cardType == 'source') {
          flag = !e.isSourcePlayed;
        } else {
          flag = !e.isDestinationPlayed;
        }
        return flag;
      },
      onAccept: (data) {
        int score = 0;
        if (data == e.id.toString()) {
          e.isMatched = true;
          score = 1;
        }
        if (cardType == 'source') {
          e.isSourcePlayed = true;
        } else {
          e.isDestinationPlayed = true;
        }
        // e.isMatched = true;
        // model.playSound("success");
        model.playVoice(cardType == "source" ? e.sourceVoice : e.matchVoice);

        model.incrementScore(score, 1, data, e.id, cardType == 'source');
      },
    );
  }

  Draggable<String> buildDraggableCards(
      MatchCard e, MatchViewModel model, String side, String cardType) {
  ...
  Container buildDraggableCard(
      MatchViewModel model, MatchCard e, String cardType) {
    return Container(
      width: 300.sp,
      height: 200.sp,
      // color: Colors.red,
  ...
  Widget buildCard(MatchViewModel model, MatchCard e, String cardType,
